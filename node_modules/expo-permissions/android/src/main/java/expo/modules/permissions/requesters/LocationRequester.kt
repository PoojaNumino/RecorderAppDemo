...
class LocationRequester(val includeBackgroundPermission: Boolean = false) : PermissionRequester {
  override fun getAndroidPermissions(): List<String> {
    ...
    if (includeBackgroundPermission && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      list.add(0, Manifest.permission.ACCESS_FINE_LOCATION)
    }
    return list
  }
  ...
  override fun parseAndroidPermissions(permissionsResponse: Map<String, PermissionsResponse>): Bundle {
    return Bundle().apply {
      ...
      scope =
        if (includeBackgroundPermission && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
          val accessBackgroundLocation = permissionsResponse.getValue(Manifest.permission.ACCESS_FINE_LOCATION)
